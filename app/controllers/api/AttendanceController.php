...
class AttendanceController extends ApiController {
    ...
    public function index() {
        ...
        if ($meetingSchedule) {
            foreach ($attendanceList as $sid => &$row) {
                if ($row['meeting'] === null) {
                    $row['meeting'] = (object) [
                        'attendance_id' => null,
                        'status' => 'Pending',
    ...
    public function update() {
        ...
        if (empty($id) && !empty($scheduleId) && !empty($serverId)) {
            ...
            if (!$id) {
                $this->error('Failed to create attendance record.', 500);
            }
        }
        ...
        if ($this->attendanceRepo->updateStatus($id, $status)) {
            ...
            $this->ok(['message' => 'Attendance updated.', 'attendance_id' => $id, 'date' => $date]);
        }
    ...
    public function downloadReport() {
        ...
        $allServers = $this->serverRepo->getAll();
